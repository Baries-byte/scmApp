<x-layoutAdmin>
    ...
    <div class="mx-5 mt-5 border-4 p-4 mb-10 md:mx-10">
        ...
        <div class=" flex flex-row w-full ">
            ...
            <div class="w-1/2">
                <h3 class="text-xl font-bold mb-3">Pilih Barang</h3>
                            ...
                            @foreach ($barangSupplier as $b)
                                <tr>
                                    ...
                                    <td class="border border-slate-600 p-1 text-center">
                                        <form action="{{ route('storeDetailPurchaseOrderSupplier') }}" method="POST">
                                            @csrf
        ...
        <div>
            <h3 class="text-xl font-bold mb-5">Barang yang dikirim</h3>
            <div class="w-full overflow-x-auto">
                <table class="table-auto border-collapse border border-slate-500 w-full">
                    <thead class="bg-gray-300">
                        <tr>
                        ...
                        @foreach ($detailPurchaseOrder as $po)
                            <tr>
                                ...
                                <td class="border border-slate-600 p-1 w-1/4">
                                    <div class="flex">
                                        <form action="{{ route('updataDetailPurchaseOrderSupplier') }}" method="POST">
                                            @csrf
                                            @method('PUT')
                                            ...
                                            <input type="hidden" name="purchase_order_id"
                                                value="{{ $purchaseOrder->id }}">
                                            <input type="hidden" name="barang_supplier_id"
                                                value="{{ $po->barangSupplier->id }}">
                                            <input type="hidden" name="harga_jual" class="text-center"
                                                value="{{ $po->barangSupplier->harga_jual }}">
                                        ...
                                        <form method="POST"
                                            action="{{ route('deleteDetailPurchaseOrderSupplier', ['detailPurchaseOrder' => $po->id]) }}"
                                            class="mx-1">
                                            @csrf
                                            @method('DELETE')
                                            <button>
        ...
        <div class="w-full flex justify-end my-2">
            <a href="{{ route('prosesBuatPOSupplier', ['purchaseOrder' => $purchaseOrder->id]) }}"
