<x-layoutAdmin>
    <x-title>
        Daftar Purchase Order dari Toko
    </x-title>

    <div class="mx-5 mt-5 border-4 p-4 md:mx-10">
        <div class="w-full flex justify-between bg-red-">
            <a href="{{ route('createPurchaseOrderSupplier') }}" class="border rounded-lg p-1 my-2 bg-green-500">Buat
                Purchase
                Order</a>
            <div>
                <form action="">
                ...
                <table class="table table-auto border-collapse border border-slate-500 w-full">
                    <thead class="bg-slate-400">
                        <tr>
                            ...
                            <th class="border border-slate-600">Nomor Surat Jalan / Invoice</th>
                        ...
                        @foreach ($purchaseOrder as $p)
                            <tr class="relative">
                                ...
                                <td class="border border-slate-600 p-1 text-center">
                                    {{ $p->kode_purchase_order }}
                                </td>
                                <td class="border border-slate-600 p-1 text-center">
                                    {{ $p->kode_purchase_order_supplier }}
                                </td>
                                <td class="border border-slate-600 p-1 text-center">
                                    {{ $p->total_item }}
                                </td>
                                ...
                                <td class="border border-slate-600 p-1">
                                    <span class="{{ $p->status == 'Ditolak' ? 'bg-red-600' : 'bg-green-400' }}">
                                        {{ $p->status }}
                                    </span>
                                </td>
                                <td class="border border-slate-600 p-1 text-center">
                                    <a href="{{ route('showPurchaseOrderSupplier', ['purchaseOrder' => $p->id]) }}">Detail
                                    </a>
                                </td>
                        @endforeach
                    </tbody>
