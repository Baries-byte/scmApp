<x-layoutAdmin>
    <x-title>
        Daftar Transaksi Penjualan Aktif
    </x-title>
    ...
    <div class="mx-5 mt-5 border-4 p-4 md:mx-10">
        ...
        <div class="mb-10" id="transaksiAktif">
            <div class="flex justify-end">
                <div class="w-1/2 flex justify-end">
                    <form action="">
                        <input type="text" class="border rounded-lg p-1 my-2" name="search_penjualan_aktif"
                            placeholder="Cari transaksi.." />
                        <button type="submit" class="bg-primary p-1 text-white rounded-lg w-20">Cari</button>
                    </form>
                </div>
            </div>
            @if (count($penjualanAktif) != 0)
                <table class="table table-auto border-collapse border border-slate-500 w-full">
                    <thead class="bg-slate-400">
                        <tr>
                            ...
                            <th class="border border-slate-600">Kode Penjualan</th>
                    ...
                    <tbody>
                        ...
                        @foreach ($penjualanAktif as $p)
                            {{-- @php
                                $value = '';
                                dd($p->status_transaksi);
                            @endphp
                            @switch($p->status_transaksi)
                                @case('DiProses')
                                    @php
                                        $value = 'bg-green-200';
                                    @endphp
                            ...
                            <tr class="relative">
                                ...
                                <td class="border border-slate-600 p-1">{{ $p->kode_penjualan }}</td>
                ...
                <!-- Pagination -->
                <div class="my-5">
                    {{ $penjualanAktif->links('vendor.pagination.tailwind') }}
                </div>
            @else
                <p class="text-center bg-red-300 ">Data transaksi penjualan aktif tidak tersedia</p>
            @endif
        </div>
    </div>
</x-layoutAdmin>
